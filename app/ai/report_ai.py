            ...
            *(
                [
                    {
                        "role": "user",
                        "content": (
                            "아래 Evidence는 실제로 수집된 네이버 검색량 데이터입니다.\n"
                            "이것은 보조 참고자료가 아니라 1순위 사실 근거입니다.\n"
                            "사용자의 주관적 답변과 Evidence가 충돌하면 Evidence를 우선하세요.\n"
                            "단, 사용자가 제공한 서비스 기본 정보(무엇을 파는지, 누구에게 파는지)는 "
                            "사실로 전제하고, Evidence는 그 시장을 해석하는 근거로 사용하세요.\n\n"
                            "Evidence 사용 규칙:\n"
                            "- 아래 표시된 Evidence ID만 사용하세요. ID를 창작하지 마세요.\n"
                            "- Evidence가 뒷받침하지 않는 내용을 단정하지 마세요.\n"
                            "- Evidence를 실제로 사용한 섹션은 evidence_ids를 반드시 채우세요. "
                            "사용했는데 ID를 누락하지 마세요.\n"
                            "- AI 해석·전략만으로 작성한 섹션은 evidence_ids를 빈 배열로 두세요.\n"
                            "- 각 Evidence의 seed 값(문제/해결/대안/추천/가격/브랜드)은 "
                            "검색어가 어느 갈래에서 나왔는지를 뜻합니다.\n\n"
                            "seed 갈래별 해석 기준:\n"
                            "- 문제: 이 시장을 검색으로 찾는 사람이 있는가\n"
                            "- 해결: 고객이 '도구'를 찾고 있는가\n"
                            "- 대안: 고객이 '사람/대행'을 찾고 있는가\n"
                            "- 추천: 지금 구매를 비교·검토 중인 고객이 얼마나 되는가\n"
                            "- 가격: 가격에 민감한 시장인가\n"
                            "- 브랜드: 우리 이름으로 찾는 사람이 있는가\n\n"
                            "반드시 갈래 간 검색량을 비교해 해석하세요.\n"
                            "예시: '대안' 갈래가 '해결' 갈래보다 검색량이 크게 높다면, "
                            "고객은 도구가 아니라 사람(대행)을 찾고 있다는 뜻입니다.\n"
                            "예시: 사용자가 쓰는 표현의 검색량이 낮고 다른 표현이 높다면, "
                            "'고객과 다른 단어를 쓰고 있다'는 사실을 명시하세요.\n\n"
                            "검색량이 낮은 것도 중요한 근거입니다. 낮다고 무시하지 말고 "
                            "'이 표현으로는 찾는 사람이 거의 없다'는 판단 근거로 사용하세요.\n\n"
                            f"{rag_context}"
                        ),
                    }
                ]
            ...
            ),
            {
                ...
                "content": (
                    ...
                    f"전체 인터뷰 문맥:\n{interview_context}\n\n"
                    "정보 우선순위:\n"
                    "1순위 서비스 기본 정보 (무엇을/누구에게 파는지 — 사실로 전제)\n"
                    "2순위 Evidence 검색량 데이터 (시장 해석의 근거)\n"
                    "3순위 사용자 인터뷰 답변 (주관적 견해)\n"
                    "사용자의 추측과 Evidence가 다르면 Evidence를 따르세요.\n\n"
            ...
            },
            {
                ...
                "content": (
                    ...
                    "- JSON 최상위 key는 위 7개만 사용하고, 모든 value는 dict 타입을 유지하세요.\n\n"
                    "★ 업종 적합성 (최우선 규칙):\n"
                    "- 서비스 정보의 업종을 먼저 확인하고, 그 업종에 실제로 존재하는 요소만 언급하세요.\n"
                    "- 업종별 사용 가능 채널:\n"
                    "  · IT/SaaS·B2B: 네이버 블로그, 네이버 검색광고, 구글 검색광고, 커뮤니티(디스콰이엇·오키·아웃스탠딩 등), 뉴스레터, 링크드인, 유튜브(설명형), X(트위터)\n"
                    "  · 오프라인 매장(F&B·뷰티·헬스케어): 네이버 플레이스, 카카오맵, 당근, 인스타그램, 지역 커뮤니티\n"
                    "  · 쇼핑몰: 네이버 쇼핑, 인스타그램, 카카오, 유튜브 쇼츠, 리뷰 플랫폼\n"
                    "  · 교육: 네이버 블로그, 카페, 유튜브, 인스타그램\n"
                    "- ★ 오프라인 매장이 아닌 서비스에는 다음을 절대 추천하지 마세요:\n"
                    "  네이버 플레이스, 카카오맵, 지역 제휴, 재방문 쿠폰, 매장 방문, 단골 고객 쿠폰, 지역 확장, 예약 시스템\n"
                    "- ★ 구독형 서비스에는 '재방문' 대신 '구독 유지/이탈 방지'라는 표현을 사용하세요.\n"
                    "- 업종에 맞지 않는 채널이나 행동을 하나라도 추천하면 리포트 전체가 무효입니다.\n\n"
                    "★ recommendations 작성 규칙 (모든 섹션 공통):\n"
                    "- 각 항목은 반드시 다음 세 요소를 포함하세요.\n"
                    "  ① 무엇을 — 구체적인 행동\n"
                    "  ② 어디에/어떤 것으로 — 채널·키워드·대상을 실명으로 명시\n"
                    "  ③ 왜 — 근거 또는 판단 이유 (Evidence가 있으면 수치를 인용)\n"
                    "- 각 항목은 2~3문장, 80자 이상으로 작성하세요.\n"
                    "- '~할 것' 한 문장으로 끝내지 마세요.\n"
                    "- 사용자가 이번 주에 실제로 착수할 수 있는 수준으로 쓰세요.\n\n"
                    "나쁜 예 (이렇게 쓰지 마세요):\n"
                    "  \"검색 최적화와 콘텐츠 생산에 집중할 것.\"\n"
                    "  \"고객 후기와 사례 축적을 통해 신뢰도를 높일 것.\"\n"
                    "  → 무엇을 어디에 왜 하는지가 없어 사용자가 실행할 수 없습니다.\n\n"
                    "좋은 예 (이 수준으로 쓰세요):\n"
                    "  \"'마케팅 대행사' 키워드로 네이버 블로그 글을 주 2편 작성하십시오. 이 키워드는 월 690회 검색되는 반면, 현재 사용 중인 '마케팅 툴'은 월 30회에 그칩니다. 고객이 실제로 쓰는 단어로 글을 써야 검색에 노출됩니다.\"\n\n"
                    "insights 작성 규칙:\n"
                    "- 동어반복을 금지합니다. ('성장 가능성은 품질에 달려있다' 같은 문장은 정보가 없습니다)\n"
                    "- 다른 서비스에 그대로 옮겨도 말이 되는 문장은 쓰지 마세요.\n"
                    "- 이 서비스에만 해당하는 내용을 쓰세요.\n"
                    "- Evidence가 있는 항목은 수치를 문장 안에 직접 넣으세요.\n"
                    "- 각 항목 1~2문장, 50자 이상.\n\n"
                    "competitor_analysis 작성 기준:\n"
                    "- 대한민국 시장 기준으로 분석하세요.\n"
                    "- 가능한 경우 실제 국내 서비스, 플랫폼, 기업명을 2~4개 언급하세요.\n"
                    "- 확실하지 않은 기업명은 단정하지 말고 '유사 경쟁군' 또는 "
                    "'간접 경쟁 채널'로 표현하세요.\n"
                    "- 해외 기업 중심 분석은 금지하세요.\n"
                    "- 각 경쟁 대상마다 경쟁사/경쟁 채널명, 강점, 약점, 우리 서비스의 "
                    "차별화 지점, 초기 창업자 관점의 대응 전략을 포함하세요.\n"
                    "- Evidence의 '대안' 갈래 검색량이 높다면, 그 대안이 실질적 경쟁자임을 "
                    "수치와 함께 명시하세요.\n\n"
                    "platform_recommendation 작성 기준:\n"
                    "- 반드시 위 '업종 적합성' 규칙을 먼저 적용하세요.\n"
                    "- 단순히 특정 플랫폼이 좋다는 수준으로 쓰지 말고 추천 우선순위 "
                    "형태로 작성하세요.\n"
                    "- 각 플랫폼마다 추천 순위, 추천 이유, 적합한 고객 단계"
                    "(인지도/관심/전환/유지), 예산이 적을 때 실행 방법, 기대 효과, "
                    "주의할 점을 포함하세요.\n"
                    "- Evidence에 검색량이 있다면, 검색 기반 채널의 우선순위 판단에 "
                    "그 수치를 근거로 사용하세요.\n\n"
                    ...
                    "- contentSeries에는 실제로 복사해 사용할 수 있고 순서대로 연결되는 게시물 초안을 기본 3개 작성하세요.\n"
                    "- 각 게시물은 objective, hook, body, 선택적 cta, hashtags, evidenceIds, 선택적 caution을 구분하세요.\n"
                    "- profileName이나 SNS 계정 ID, 실제 운영 계정을 만들지 마세요. brandDisplayName은 입력된 서비스명을 변형하지 마세요.\n"
                    "- evidenceIds에는 위 Evidence에 제공된 ID만 사용하고, 근거가 없으면 빈 배열로 작성하세요.\n"
                    "- 성과를 보장하거나 허위 고객 발언, 확인되지 않은 통계를 작성하지 마세요."
                    "\n\n항목별 추가 품질 기준:\n"
                    ...
                    "- marketing_strategy: executionPhases를 30일 내 순차 단계로 추가하고 KPI마다 calculation, measurementWindow, decisionRule을 포함하세요. contentSeries는 id, platform, platformLabel, brandDisplayName, seriesTitle, cadence, posts를 사용하고 posts는 id, sequence, dayLabel, objective, hook, body, cta, hashtags, evidenceIds, caution을 사용하세요.\n"
